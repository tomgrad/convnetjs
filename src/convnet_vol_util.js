...
function augment(V, crop, dx, dy, fliplr = false) {
  // note assumes square outputs of size crop x crop
  if(dx === undefined) dx = randi(0, V.sx - crop);
  if(dy === undefined) dy = randi(0, V.sy - crop);
...
function img_to_vol(img, convert_grayscale = false) {
  ...
  for(let i=0;i<pv.length;i++) { x.w[i] = pv[i]; } // keep the Float64Array buffer
